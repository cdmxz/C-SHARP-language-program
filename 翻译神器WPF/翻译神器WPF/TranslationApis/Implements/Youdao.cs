...
namespace 翻译神器WPF.TranslationApi.Implements
{
    ...
    {
        public string ApiChineseName { get; } = ApiNames.GetApiChineseNameByApiCode(ApiCodes.YouDao);

        private YoudaoKey youdaoKey;

        public Youdao(YoudaoKey key)
        {
            youdaoKey = key;
        }
        ...
        public void PictureTranslate(Image img, string from, string to, out string src_text, out string dst_text)
        {
            ...
            string q = TranslateUtils.ImageToBase64(img);
        ...
        /// <returns>译文</returns>
        /// <exception cref="Exception"></exception>
        public string TextTranslate(string srcText, string from, string to)
        {
            if (youdaoKey.IsEmpty)
            ...
            string result = Post(url, str);
            TranslationJson? list = JsonSerializer.Deserialize<TranslationJson>(result);
            if (list is null)
                throw new Exception("解析Json数据失败");
            if (list.errorCode != "0")
                throw new Exception("错误代码：" + list.errorCode);
        ...
        {
            PictureTranslate(img, LangDict["中文"], LangDict["英文"], out _, out _);
        }
        ...
        {
            return TranslateUtils.GetUpperCaseMd5(str);
        }
