...
namespace 翻译神器WPF.TranslationApi.Implements
{
    public class Tengxun : ITranslateApi
    {
        ...
        public string ApiChineseName { get; } = ApiNames.GetApiChineseNameByApiCode(ApiCodes.TengXun);

        private readonly TmtClient client;
        private readonly TengxunKey tengxunKey;

        public Tengxun(TengxunKey tk)
        {
            tengxunKey = tk;
            Credential cred = new();
        ...
        {
            PictureTranslate(img, LangDict["简体中文"], LangDict["英语"], out _, out _);
        }
        ...
        public void PictureTranslate(Image img, string from, string to, out string src_text, out string dst_text)
        {
            ...
            ImageTranslateRequest req = new()
            {
                ...
                Data = TranslateUtils.ImageToBase64(img),
                Source = from,
                Target = to,
                ProjectId = 0
            };
            ...
            var list = JsonSerializer.Deserialize<PictureTranslationJson>(result);
            if (list is null)
            ...
            src_text = sLen != -1 ? src_text[..sLen] : src_text;
            dst_text = dLen != -1 ? dst_text[..dLen] : dst_text;
        }
        ...
        public void TextTranslate(string srcText, string from, string to, out string src_text, out string dst_text)
        {
            ...
            var list = JsonSerializer.Deserialize<TranslationJson>(result);
            if (list is null)
                throw new Exception("解析Json数据失败");
        ...
        {
            return [.. LangDict.Keys.Where((key) => !key.Equals("自动识别"))];
        }
