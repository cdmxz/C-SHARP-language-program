...
namespace 翻译神器WPF.Util
{
    internal class ProcessHelper
    {
        ...
        public static bool IsRun(out Process? process)
        {
            ...
            // 遍历进程列表
            var pros = Process.GetProcessesByName(current.ProcessName);
            foreach (var p in pros)
            {
                if (p.Id != current?.Id)// 忽略当前进程   
                {
                    //// 判断要打开的进程和已经存在的进程是否来自同一路径                                                                                                                        
                    //if (Path.GetDirectoryName(current?.MainModule?.FileName) == Path.GetDirectoryName(p.MainModule?.FileName))
                    //{
